"""Formata o resumo semanal como mensagem de texto para WhatsApp (franqueadora)."""
...
def compose(data: dict[str, Any], data_inicio: date, data_fim: date) -> str:
    ...
    formatada para WhatsApp com o resumo semanal.
    """
    inicio_str = data_inicio.strftime("%d/%m")
    fim_str = data_fim.strftime("%d/%m/%Y")
    lines = []

    # -- Cabecalho ---------------------------------------------------------------
    lines.append(f"🟢 *RESUMO SEMANAL VIP* 📊 {inicio_str} - {fim_str}")
    lines.append(_sep())

    # -- Faturamento semanal -----------------------------------------------------
    fat = data.get("faturamento")
    lines.append("💰 *FATURAMENTO VISÃO CAIXA SEMANAL*")
    if fat:
        ...
        if fat.get("meta_proporcional_rede") and fat.get("pct_meta_rede"):
            lines.append(
                f"Meta semanal: {_fmt_brl(fat['meta_proporcional_rede'])} → *{_fmt_pct(fat['pct_meta_rede'])}*"
            )
    ...
    lines.append(_section("📅 *AGENDA SEMANAL*"))
    if agenda:
        lines.append(
    ...
    if perfex:
        lines.append(
            f"Novos leads na semana: *{perfex['novos_periodo']}* | "
    ...
    lines.append("_Barbearia VIP — Resumo semanal automático_")
